...
function ambrox_import_files() {
	...
	$demoImg = '<img src="'. AMBROX_DEMO_DIR_URI  .'screen-image.jpg" alt="'.esc_attr__('Demo Preview Imgae','ambrox').'" />';

    return array(
        array(
            'import_file_name'             => esc_html__('Ambrox Demo','ambrox'),
            'local_import_file'            =>  AMBROX_DEMO_DIR_PATH  . 'ambrox-demo.xml',
            'local_import_widget_file'     =>  AMBROX_DEMO_DIR_PATH  . 'ambrox-widgets-demo.json',
            'local_import_redux'           => array(
                array(
                    'file_path'   =>  AMBROX_DEMO_DIR_PATH . 'redux_options_demo.json',
...
function ambrox_demo_import_custom_scripts(){
	if( isset( $_GET['page'] ) && $_GET['page'] == 'ambrox-demo-import' ){
		// style
		wp_enqueue_style( 'ambrox-demo-import', AMBROX_DEMO_DIR_URI.'css/ambrox.demo.import.css', array(), '1.0', false );
	}
}
